...
function parseJSONLD($){
  ...
  $('script[type="application/ld+json"]').each((_,el)=>{
    try{
      const node = JSON.parse($(el).contents().text());
...
function heuristics($){
  ...
  const loc = $('a[href*="map"],[data-cy="ad-location"]').first().text().trim()
           || $('h2:contains("lokalizacja"),h3:contains("lokalizacja")').next().text().trim();
...
module.exports = async (req,res)=>{
  ...
  if(req.method!=='POST') return res.status(405).json({error:'Use POST { url, survey }'});
